...
class WorkflowSimulator:
    ...
    def simulate_workflow(self, workflow_data: Dict[str, Any]) -> SimulationResult:
        ...
        try:
            ...
            # Simulate execution
            execution_result = self._simulate_execution(groups, dependency_graph)
    ...
    def _calculate_job_counts(self, groups: List[GroupInfo], 
        ...
        for group in groups:
            if group.input_events > 0:
                # Calculate how many jobs needed: ceil(RequestNumEvents / GroupInputEvents)
                group.job_count = math.ceil(request_num_events / group.input_events)
            else:
                group.job_count = 1
    ...
    def _simulate_execution(self, groups: List[GroupInfo], 
        ...
        for group in groups:
            ...
            # Create jobs for this group
            for job_idx in range(group.job_count):
                job_id = f"{group.group_id}_job_{job_idx + 1}"
                job_wallclock = self._calculate_job_wallclock_time(group, batch_size)
        ...
        while execution_queue:
            current_group_id = execution_queue.popleft()
            if current_group_id in completed_groups:
                continue
            
            # Execute all jobs for this group
            group_jobs = [job for job in jobs if job.group_id == current_group_id]
            group_execution_time = self._execute_group_jobs(
                group_jobs, current_time, execution_log
            )
            
            # Update group status
            group_status[current_group_id] = 'completed'
            completed_groups.add(current_group_id)
            current_time += group_execution_time
    ...
    def _execute_group_jobs(self, group_jobs: List[JobInfo], 
        ...
        """Execute all jobs in a group and return total execution time."""
        if not group_jobs:
            return 0.0
        
        # Jobs in a group execute sequentially
        current_time = start_time
        max_job_time = 0.0
        
        for job in group_jobs:
            job.start_time = current_time
            job.end_time = current_time + job.wallclock_time
