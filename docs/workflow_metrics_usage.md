...
- **`simulation_stats`**: Distribution statistics over all jobs for use in visualizations. Each key (e.g. `job_overhead_secs`, `job_overhead_cpu_time`, `batch_size` for events per job) has `mean`, `std`, `median`, `min`, `max`, and `n`, computed from the full job list.
