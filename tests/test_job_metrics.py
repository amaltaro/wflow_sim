...
class TestJobMetricsCalculator:
    ...
    def test_calculate_job_statistics_empty_jobs(self):
        """Test job statistics calculation with empty jobs list."""
        calculator = JobMetricsCalculator()

        stats = calculator.calculate_job_statistics([])

        assert stats['total_cpu_time'] == 0.0
        assert stats['total_write_local_mb'] == 0.0
        assert stats['total_write_remote_mb'] == 0.0
        assert stats['total_read_remote_mb'] == 0.0
        assert stats['total_network_transfer_mb'] == 0.0
        assert stats['average_cpu_time_per_job'] == 0.0
        assert stats['average_write_local_mb_per_job'] == 0.0
        assert stats['average_write_remote_mb_per_job'] == 0.0
        assert stats['average_read_remote_mb_per_job'] == 0.0
        assert stats['average_network_transfer_mb_per_job'] == 0.0

    def test_calculate_job_statistics_single_job(self):
        """Test job statistics calculation with single job."""
        calculator = JobMetricsCalculator()

        # Create a mock job with metrics
        class MockJob:
            def __init__(self):
                self.total_cpu_time = 1000.0
        ...
        job = MockJob()
        stats = calculator.calculate_job_statistics([job])

        assert stats['total_cpu_time'] == 1000.0
    ...
    def test_calculate_job_statistics_multiple_jobs(self):
        """Test job statistics calculation with multiple jobs."""
        calculator = JobMetricsCalculator()
        ...
        # Totals
        assert stats['total_cpu_time'] == 4500.0
        ...
        # Averages
        assert stats['average_cpu_time_per_job'] == 1500.0
